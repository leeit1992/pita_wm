 ...
 var rootLength = root.length;	
	...
	if( hash.substr(0,rootLength) != root ){
		return;						
		 ...
		 var correction = 50;
		 var headerH = $('nav').outerHeight()+correction+50;
		 hash = $(window.location).attr('hash'); 
	     url = hash.replace(/[#\!]/g, '' ); 
		...
		function showProject(){
			if(content==false){
					...
					projectContainer.animate({opacity:1,height:wrapperHeight}, function(){
				        $("#ajax-section").fitVids();
						scrollPostition = $('html,body').scrollTop();
						projectNav.fadeIn();
						exitProject.fadeIn().css('display', 'inline-block');
						content = true;
					});
     ...
     /* LINKING TO PREIOUS AND NEXT PROJECT VIA PROJECT NAVIGATION */
	  $('#nextProject a').on('click',function () {											   							   
	    ...
	    /* CLOSE PROJECT */
	    $('.work-item .box-view').on('click',function () {
	        $('#ajax-section').css('padding-top','140px');
	    });
	    $('#closeProject a').on('click',function () {
	        $('#ajax-section').css('padding-top','0px');
